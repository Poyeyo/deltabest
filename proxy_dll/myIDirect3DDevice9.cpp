...
ULONG myIDirect3DDevice9::Release(void)
{
	...
	// Call the rendering plugins so that they can release/delete own objects
	std::vector<D3D9_CALLBACK_T>::const_iterator callback_iterator;
	for ( callback_iterator = g_release_callbacks.begin(); callback_iterator != g_release_callbacks.end(); callback_iterator++ )
	{
		(*callback_iterator)( this ) ;
	}
...
HRESULT myIDirect3DDevice9::Present(CONST RECT* pSourceRect,CONST RECT* pDestRect,HWND hDestWindowOverride,CONST RGNDATA* pDirtyRegion)
{
    ...
    // Let the rendering plugins have a last go before the flip happens
	std::vector<D3D9_CALLBACK_T>::const_iterator callback_iterator;
	for ( callback_iterator = g_present_callbacks.begin(); callback_iterator != g_present_callbacks.end(); callback_iterator++ )
	{
		(*callback_iterator)( this ) ;
	}
...
{
    // Notify the rendering plugins that the scene is about to finish
	std::vector<D3D9_CALLBACK_T>::const_iterator callback_iterator;
	for ( callback_iterator = g_endscene_callbacks.begin(); callback_iterator != g_endscene_callbacks.end(); callback_iterator++ )
	{
		(*callback_iterator)( this ) ;
	}
